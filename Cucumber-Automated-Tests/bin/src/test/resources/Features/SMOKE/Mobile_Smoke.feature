...
Feature: Basic functionality which check the health of website after every deployment
  ...
  @checkoutNewTradeAccount @live @mobsmoketest @cicd
  ...
  @SearchLookAhead @guest @live @mobsmoketest  @cicd
  ...
  @allfilter @guest @live @mobsmoketest  @cicd
  Scenario: Validate filter functionality is working fine
    Given web site is launched successfully and user "does" accept the cookies
    When I navigate to "c/passive-components/resistors-fixed-value/through-hole-resistors" page
    And I wait for "4" seconds
    And I verify element "Filters.searchWithinTermBox_textbox" is present on page
    And I get the Products found count before applying filters
    And I click on refine results button to view the filters options "Filters.refineResults_button"
  ...
  @guestcheckoutUsingCC @mobsmoketest  @cicd
  ...
  @checkoutUsingExistingTradeAccount @live @mobsmoketest  @cicd
  ...
  @orderHistory @checkoutNewTradeAccount @mobsmoketest
