...
public class CosmeticSteps extends WebDriverConfig {
    ...
    public void getTextColor(String Value) throws Exception {
        ...
        Assert.assertTrue(
            ...
            colorValue.trim().equalsIgnoreCase(hexColor)
        );
    }
    ...
    public void verifyBackgroundColor(String Value) throws Exception {
        String[] textColor = Value.split(",");
        String color = textColor[0], colorValue = textColor[1], locator = textColor[2];
        String ColorOfContainer = uiMethods.verifyBackgroundColor(
            color,
            colorValue,
            locator
        );
        Assert.assertTrue(
            "The color of the Background is not matching",
    ...
    public void verifyBorderColor(String Value) throws Exception {
        ...
        Assert.assertTrue(
            "The color of the border is not matching",
            ColorOfContainer.equalsIgnoreCase(colorValue.trim())
        );
    }
    ...
    public void elementIsPresentInElement(String loactorToSearch, String locator)
        throws Exception {
    ...
    // ("class=\"cadViewerTitle\"") in Obj class
    @And(
        "^.*verify element \"([^\"]*)\" is not visible in accordian element \"([^\"]*)\"$"
    )
    public void elementIsNOTPresentInElement(String loactorToSearch, String locator)
        throws Exception {
        Boolean value = uiMethods.verifyPresenceOfXpath(loactorToSearch, locator);
        Assert.assertFalse(value);
    }
    ...
    public void verifyAlignmentOfText(String locator, String expectedAlignment)
        throws Exception {
        String ActualAlignment = uiMethods.verifyAlignmentOfText(
            locator,
            expectedAlignment
        );
        Assert.assertTrue(
            "The Alignment of text matches the expected",
            ActualAlignment.trim().equalsIgnoreCase(expectedAlignment.trim())
        );
    }

    @And("^.*font size of text \"([^\"]*)\" as \"([^\"]*)\"$")
    public void verifyFontSizeOfText(String locator, String ExpectedFontSize)
        throws Exception {
        String ActualFontSize = uiMethods.verifyFontSizeOfText(locator, ExpectedFontSize);
        Assert.assertTrue(
            "Font Size of text doesn't matches the expected",
    ...
    @And(
        "^.*verify if none of the Horizontal & Vertical scroll bar are present in a page$"
    )
    ...
    public void verifyListContainsText(String locators, DataTable table)
        throws Exception {
        actionMethods.waitForElementToAppear(
            this.context.constants.getMediumWaitSeconds(),
            locators
        ...
        for (WebElement element : actWebElementList) {
            actProdDesrList.add(element.getText().replace(":", "").replace("：", ""));
        }
        ...
        // Assert.assertTrue("Price coloumn is missing",
        // actProdDesrList.get(5).contains("Price"));
        Assert.assertTrue(
            "Product description is wrong",
            actProdDesrList.containsAll(expProdDesrListTranslated)
        );
    }
    ...
    public void verifyElementOnlyDisplayedInspecificRegionStore(
        String locator,
        String acceptedList
    ) throws Exception {
     ...
     */
    @Then(
        "^.*verify element \"([^\"]*)\" is present except in \"([^\"]*)\" region/store.*$"
    )
    public void verifyElementDisplayedExceptInSpecificRegionStore(
        String locator,
        String exceptList
    ) throws Exception {
    ...
    @And(
        "^.*verify the message \"([^\"]*)\" is present in multiple tags using innertext in page \"([^\"]*)\"$"
    )
    public void verifyTextUsingInnerText(String value, String locator) throws Exception {
        ...
        Assert.assertTrue(
            "Partial text matching is failing",
    ...
    public void verifyPartialText(String value, String locator) throws Exception {
        ...
        logger.info("Actual text obtained from the page: " + act_Value);
        Assert.assertTrue(
            "Partial string is not present in the expected message",
            act_Value.contains(exp_Value)
        );
    }
    ...
    // translated text for non-English store
    @And(
        "^.*verify text.* \"([^\"]*)\" in page \"([^\"]*)\" with attribute \"([^\"]*)\"$"
    )
    public void verifyTextwithAttribute(
        String ExpectedText,
        String locator,
        String attribute
    ) throws Exception {
    ...
    @And("^.*verify current url contains \"([^\"]*)\".*$")
    public void verify_current_url_contains_string(String text) throws Exception {
    ...
    public void verifyTextFromExcelFile(String KeyName, String locator, String FileName)
        throws Exception {
        String path =
    ...
    @Then(
        "^.*verify text.* in page \"([^\"]*)\" is present from file \"([^\"]*)\" except in \"([^\"]*)\" region/store.*$"
    )
    public void verifyTextFromExcelFileSpecificRegionStore(
        String locator,
        String FileName,
        String exceptList
    ) throws Exception {
    ...
    public void verifyTextForTwoElementsOnWeb(String locator1, String locator2)
        throws Exception {
    ...
    @And(
        "^.*verify text.* \"([^\"]*)\" in page \"([^\"]*)\" for \"([^\"]*)\" region/store$"
    )
    public void verifyTextInASpecificRegion(String value, String locator, String region)
        throws Exception {
        String regionArray[] = region.toLowerCase().split(",");
        String regionName = getRegionName().toLowerCase();
        String store = getStore().toLowerCase();
        if (
            Arrays.asList(regionArray).contains(regionName) ||
            Arrays.asList(regionArray).contains(store)
        ) {
            String exp_Value = storeText.getStoreText(value);
            String act_Value = actionMethods.getValue(locator);
    ...
    public void getTextColorOfElementsInList(String locator, String Value)
        throws Exception {
        boolean flag = false;
        ...
        for (WebElement e : elementsInList) {
            ...
            flag =
                (
                    firstColor.equalsIgnoreCase(hexColor) ||
                    secondColor.equalsIgnoreCase(hexColor)
                );
    ...
    public void numbOfProducts(String locator) throws Exception {
        ...
        this.context.scenario.log(
                "No. of products found in page with the locator: " + countOfProducts
            );
    }

    @And("^.*verify count of elements present as \"([^\"]*)\" in page \"([^\"]*)\"$")
    public void verifyCountOfElements(int expectedCount, String locator)
        throws Exception {
