...
public class RegistrationPageSteps extends WebDriverConfig {
    ...
    public HashMap<String, String> getMandatoryFieldsInRegistraionPageForAGivenStore(
        String FileName
        ...
        for (Map.Entry<String, String> keyEntry : keywordMap.entrySet()) {
            for (Map.Entry<String, String> fieldEntry : mandatoryInputFieldMap.entrySet()) {
                if (keyEntry.getKey() == fieldEntry.getKey()) {
                    ...
                    if (
                        value.equalsIgnoreCase("MANDATORY_INPUT_FIELD_LABELS") &&
                        key.equalsIgnoreCase("FEATURE_FILE_KEYWORDS")
                    ) {} else if (value == null || value.equals("skip")) {} else {
    ...
    @Then(
        "^.*verify error message heading has \"([^\"]*)\" on \"([^\"]*)\" for \"([^\"]*)\" data entered.*$"
    )
    public void verifyErrorMsgHeadingWithEitherInvalidOrBlankInputField(
        String locator,
        ...
        if (
            customerInfo.equals("BLANK") ||
            customerInfo.equals("INVALID") &&
            !customerInfo.equals("BLANK_AND_INVALID")
        ) {
            if (errorMessageHeading.equals("Please correct fields")) {
                Assert.assertTrue(
                    errorMessageHeading.equals(
                        storeText.getStoreText("Please correct the following fields:")
                    )
                );
            } else if (errorMessageHeading.equals("Please enter required fields")) {
                Assert.assertTrue(
                    errorMessageHeading.equals(
                        storeText.getStoreText(
    ...
    @Then(
        "^.* verify if the existing user error \"([^\"]*)\" displayed on page when \"([^\"]*)\" is entered using \"([^\"]*)\" file"
    )
    public void verifyRegistrationWithExistingUserError(
        String locator,
        String customerInfo,
        String fileName
    ) throws Exception {
    ...
    @Then(
        "^.*verify if the invalid input fields \"([^\"]*)\" are displayed in the error message \"([^\"]*)\".*$"
    )
    public void verifyInvalidInputFieldsInErrorMessage(
        String invalidInputFields,
        String locator
    ) throws Exception {
        ...
        for (int i = 0; i < invalidFields.length; i++) {
            if (invalidFields[i].equalsIgnoreCase("email")) {
                expected_err_input_fields.add(
                    storeText.getStoreText("Invalid email address format.").trim()
                );
            }
            ...
            if (invalidFields[i].equalsIgnoreCase("password")) {
                expected_err_input_fields.add(
                    storeText
                        .getStoreText(
                            "Password entered does not meet password policy requirements. " +
                            "It must be 8-20 characters and contain at least 1 upper case letter, 1 lower case letter, a number and a special character"
                        )
                        .trim()
                );
            }
    ...
    @Then(
        "^user verifies if \"([^\"]*)\" input fields displayed on blank error message \"([^\"]*)\" when user enters data from \"([^\"]*)\" file$"
    )
    public void verifyBlankInputFieldsInErrorMessage(
        String blankInputFields,
        String locator,
        String fileName
    ) throws Exception {
        List<WebElement> err_message = getMutipleLocators(locator);
        HashMap<String, String> mandatoryInputFields = getMandatoryFieldsInRegistraionPageForAGivenStore(
            fileName
        );
        ...
        if (!blankInputFields.equalsIgnoreCase("all")) {
            ...
            for (int i = 0; i < blankFields.length; i++) {
                expected_blank_input_fields.add(
                    storeText
                        .getStoreText(mandatoryInputFields.get(blankFields[i]))
                        .trim()
                );
                Collections.sort(expected_blank_input_fields);
            }
        } else if (blankInputFields.equalsIgnoreCase("all")) {
            List<String> expectedBlankFieldList = new ArrayList<String>(
                mandatoryInputFields.values()
            );
        ...
        Assert.assertTrue(
            actual_blank_input_fields.containsAll(expected_blank_input_fields)
        );
    }

    @And(
        "^user enters customer data with \"([^\"]*)\" details in registration page from \"([^\"]*)\" file$"
    )
    public void enterCustInfoInRegistrationPageFromExcelFile(
        String customerInfo,
        String FileName
    ) throws Exception {
        String path = FilePath + FileName;
        ...
        for (int row = 1; row < totalRows; row++) {
            ...
            if (customerInfo.equals(cellValue.trim())) {
                ...
                String vatRegistrationNumber = testData
                    .get(row)
                ...
                if (
                    emailAddress == null || emailAddress.equalsIgnoreCase("skip")
                ) {} else {
                    actionMethods.enterText(
                        emailAddress,
                        "Register.emailAddress_textbox"
                    );
                }
                ...
                if (
                    phoneExtNumber == null || phoneExtNumber.equalsIgnoreCase("skip")
                ) {} else {
                    actionMethods.enterText(
                        phoneExtNumber,
                        "Register.extensionNumber_textbox"
                    );
                }
                ...
                if (primaryJob == null || primaryJob.equalsIgnoreCase("skip")) {} else {
                    actionMethods.selectValueByValue(
                        primaryJob,
                        "Register.primaryRole_dropdown"
                    );
                }
                if (primaryUse == null || primaryUse.equalsIgnoreCase("skip")) {} else {
                    actionMethods.selectValueByValue(
                        primaryUse,
                        "Register.primaryUse_dropdown"
                    );
                }
                if (
                    accountNumber == null || accountNumber.equalsIgnoreCase("skip")
                ) {} else {
                    actionMethods.enterText(
                        accountNumber,
                        "Register.accountNumber_textbox"
                    );
                }
                if (
                    mobileNumber == null || mobileNumber.equalsIgnoreCase("skip")
                ) {} else {
                    actionMethods.enterText(
                        mobileNumber,
                        "Register.mobileNumber_textbox"
                    );
                }
    ...
    @Then(
        "^.* verifies the name and address input labels \"([^\"]*)\" on the registration form using \"([^\"]*)\" file$"
    )
    public void verifyNameAndAddressInputLabels(String locator, String fileName)
        throws Exception {
        String path = FilePath + fileName;
        List<Map<String, String>> testData = ExcelUtil.getData(path, getStore());
        ...
        for (int row = 0; row < totalRows; row++) {
            String cellValue = testData.get(row).get("scenario");
            if (
                !(cellValue == null) && cellValue.equals("NAME_AND_ADDRESS_FORM_FIELDS")
            ) {
                ...
                for (Map.Entry<String, String> Entry : map.entrySet()) {
                    ...
                    if (
                        ...
                        if (
                            value.equals("Ext. Number") ||
                            value.equals("Zip / Postal Code") ||
    ...
    @Then(
        "^.* verifies the contact preference details \"([^\"]*)\" on the registration form using \"([^\"]*)\" file in \"([^\"]*)\" regions$"
    )
    public void verifyContactPreferenceDetails(
        String locator,
        String fileName,
        String regions
    ) throws Exception {
