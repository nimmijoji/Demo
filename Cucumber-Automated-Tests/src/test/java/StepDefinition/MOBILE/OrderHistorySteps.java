...
public class OrderHistorySteps extends WebDriverConfig {
    ...
    public void save_order_code(String loc) throws Exception {
        this.context.PLP_FIRST_ORDER_CODE = actionMethods.getValue(loc);
        this.context.scenario.log(
                "Value of Ordercode obtained: " + this.context.PLP_FIRST_ORDER_CODE
            );
    }
    ...
    public void verify_reordered_product() throws Exception {
        ...
        System.out.println("Expected OrderCode: " + this.context.PLP_FIRST_ORDER_CODE);
        if (ordercodes.size() > 1) {
            for (int i = 0; i < total_ordercodes; i++) {
                ...
                if (
                    ordercodes
                        .get(i)
                        .getText()
                        .contains(this.context.PLP_FIRST_ORDER_CODE)
                ) {
            ...
            String actual_ordercode = driver
                ...
                .getText()
                .trim();
            Assert.assertEquals(this.context.PLP_FIRST_ORDER_CODE, actual_ordercode);
        }
    }
